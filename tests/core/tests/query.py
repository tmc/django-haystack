...
from haystack.backends import QF, BaseSearchQuery
...
class QFTestCase(TestCase):
    ...
    def test_repr(self):
        self.assertEqual(repr(QF(foo='bar')), '<QF: AND foo__exact=bar>')
        self.assertEqual(repr(QF(foo=1)), '<QF: AND foo__exact=1>')
        self.assertEqual(repr(QF(foo=datetime.datetime(2009, 5, 12, 23, 17))), '<QF: AND foo__exact=2009-05-12 23:17:00>')
...
class BaseSearchQueryTestCase(TestCase):
    ...
    def test_get_count(self):
        self.bsq.add_filter(QF(foo='bar'))
    ...
    def test_add_filter(self):
        ...
        self.bsq.add_filter(QF(foo__lt='10'))
        
        self.bsq.add_filter(~QF(claris='moof'))
        
        self.bsq.add_filter(QF(claris='moof'), use_or=True)
        
        self.assertEqual(repr(self.bsq.query_filter), '<QF: OR ((foo__exact=bar AND foo__lt=10 AND NOT (claris__exact=moof)) OR claris__exact=moof)>')
        
        self.bsq.add_filter(QF(claris='moof'))

        self.assertEqual(repr(self.bsq.query_filter), '<QF: AND (((foo__exact=bar AND foo__lt=10 AND NOT (claris__exact=moof)) OR claris__exact=moof) AND claris__exact=moof)>')
...
class SearchQuerySetTestCase(TestCase):
    ...
    def test_auto_query(self):
        ...
        self.assert_(isinstance(sqs, SearchQuerySet))
        self.assertEqual(repr(sqs.query.query_filter), '<QF: AND (content__exact=test AND content__exact=search AND NOT (content__exact=stuff))>')
        
        sqs = self.bsqs.auto_query('test "my thing" search -stuff')
        self.assert_(isinstance(sqs, SearchQuerySet))
        self.assertEqual(repr(sqs.query.query_filter), '<QF: AND (content__exact=my thing AND content__exact=test AND content__exact=search AND NOT (content__exact=stuff))>')
        
        sqs = self.bsqs.auto_query('test "my thing" search \'moar quotes\' -stuff')
        self.assert_(isinstance(sqs, SearchQuerySet))
        self.assertEqual(repr(sqs.query.query_filter), '<QF: AND (content__exact=my thing AND content__exact=moar quotes AND content__exact=test AND content__exact=search AND NOT (content__exact=stuff))>')
        
        sqs = self.bsqs.auto_query('test "my thing" search \'moar quotes\' "foo -stuff')
        self.assert_(isinstance(sqs, SearchQuerySet))
        self.assertEqual(repr(sqs.query.query_filter), '<QF: AND (content__exact=my thing AND content__exact=moar quotes AND content__exact=test AND content__exact=search AND content__exact="foo AND NOT (content__exact=stuff))>')
        
        sqs = self.bsqs.auto_query('test - stuff')
        self.assert_(isinstance(sqs, SearchQuerySet))
        self.assertEqual(repr(sqs.query.query_filter), '<QF: AND (content__exact=test AND content__exact=- AND content__exact=stuff)>')
